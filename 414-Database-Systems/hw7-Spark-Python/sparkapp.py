...
def main(argv):
    ...
    rA.repartition(1).write.mode("overwrite").format("csv").save(outputfile + "QA")
...
def QA(spark, dataFile):
    ...
    # r2 = rdd.map(lambda t: Row(t[DEST_CITY_NAME]).distinct()) . . .
    # there were some alternative, interesting ways to do this one that we discussed in section but I went for this one
    return r1
...
def QB(spark, dataFile):
    ...
    r1 = d.where(d.colRegex("cancelled") != 1) \
        .select(d.colRegex("origincityname"), d.colRegex("month")) \
        .groupBy(d.colRegex("origincityname"), d.colRegex("month")) \
        .count()
    return r1
...
def QC(spark, dataFile):
    ...
    # : your code here
    # errors
    # d.filter(d.colRegex("depdelay") is not None and d.colRegex("depdelay") != 0)
    # .where(d.colRegex("depdelay") is not None and d.colRegex("depdelay") != 0) \\
    # .avg(d.colRegex("depdelayminutes"))

    d.createOrReplaceTempView("flights")

    r1 = d.select(d.colRegex("origincityname"), d.colRegex("depdelayminutes")) \
